        ...
        if True:
            ...
            if source_dir == dest_dir:
                ...
                try:
                    ...
                    #shutil.copytree(".", temp_data, ignore=shutil.ignore_patterns('* '))
                except (shutil.Error, PermissionError) as exc:
                    errors = exc.args[0]
                    LOGGER.warning(
                        _('The following files could not be copied:\n%s'), [x[0] for x in errors]
                    )
                    for error in errors:
                        try:
                            os.remove(error[1])
                        except:
                            LOGGER.warning(_('Could not delete: %s'), error[1])
            ...
            if source_dir == dest_dir:
                ...
                total_bytes, total_files = make_manifests(
                    "data", processes, algorithms=checksums, encoding=encoding,
                    dest_dir = dest_dir, rel_dir = 'data'
                )
...
def make_manifests(data_dir, processes, algorithms=DEFAULT_CHECKSUMS, encoding="utf-8", dest_dir = None, rel_dir = None):
    LOGGER.info(
    ...
    if dest_dir:
        data_dir = os.path.join(dest_dir, data_dir)
        files = [os.path.relpath(i, data_dir) for i in _walk(data_dir)]
    else:
        dest_dir = os.getcwd()
        data_dir = os.path.relpath(data_dir)
        files = _walk(data_dir)

    manifest_line_generator = partial(generate_manifest_lines, algorithms=algorithms, rel_dir = rel_dir)

    
    if processes > 1:
        pool = multiprocessing.Pool(processes=processes)
        #checksums = pool.map(manifest_line_generator, _walk(data_dir))
        checksums = pool.map(manifest_line_generator, files)
        pool.close()
        pool.join()
    else:
        #checksums = [manifest_line_generator(i) for i in _walk(data_dir)]
        checksums = [manifest_line_generator(i) for i in files]
...
def generate_manifest_lines(filename, algorithms=DEFAULT_CHECKSUMS, rel_dir = None):
